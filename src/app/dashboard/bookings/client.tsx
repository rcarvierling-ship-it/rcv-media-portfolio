...
function ProjectCard({ booking, stage, onMove, onSetStatus, onUpdatePrice, albums, onLinkAlbum, isProcessing }: any) {
  ...
  return (
    <motion.div layout className="bg-zinc-900/50 backdrop-blur-md border border-white/5 p-6 rounded-sm relative group hover:border-white/20 transition-all">
       ...
       <div className="flex justify-between items-start mb-6"><div><h4 className="text-lg font-black uppercase tracking-tight text-white mb-1 leading-none">{booking.name}</h4><span className="text-[10px] font-black text-blue-500 uppercase tracking-widest">{booking.shoot_type}</span></div><button onClick={() => onSetStatus(booking.id, 'canceled')} className="p-2 text-zinc-600 hover:text-red-500 transition-colors"><Trash2 size={16} /></button></div>
       ...
       <div className="space-y-5"><div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest"><div className="flex items-center gap-2 text-zinc-500"><Calendar size={12} /><span>{booking.session_date}</span></div><div className="flex items-center gap-1 text-emerald-500"><DollarSign size={14} /><input type="number" className="bg-transparent w-20 text-white font-black text-sm outline-none" defaultValue={booking.total_amount || 0} onBlur={(e) => onUpdatePrice(booking.id, parseFloat(e.target.value))} /></div></div><div className="flex gap-2 pt-4 border-t border-white/5"><button disabled={stage.id === 'lead'} onClick={() => { const idx = STAGES.findIndex(s => s.id === stage.id); onMove(booking.id, STAGES[idx-1].id); }} className="flex-1 py-3 bg-zinc-800 text-white rounded-sm disabled:opacity-20 flex items-center justify-center hover:bg-zinc-700 transition-colors"><ChevronRight size={16} className="rotate-180" /></button><button onClick={() => { const idx = STAGES.findIndex(s => s.id === stage.id); onMove(booking.id, STAGES[idx+1].id); }} className={`flex-[3] py-3 rounded-sm flex items-center justify-center font-black uppercase text-[10px] tracking-[0.2em] gap-2 transition-all ${stage.id === 'delivered' ? 'bg-emerald-500 text-black hover:bg-emerald-400' : 'bg-white text-black hover:bg-zinc-200'}`}>{stage.id === 'lead' ? 'CONFIRM LEAD' : stage.id === 'delivered' ? 'PAYMENT RECEIVED' : 'NEXT STAGE'} <ChevronRight size={16} /></button></div></div>
