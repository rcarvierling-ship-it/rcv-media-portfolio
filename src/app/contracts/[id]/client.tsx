                  ...
                  <div className="mt-20 pt-12 border-t border-white/5">
                     <div className="flex items-center gap-6">
                        ...
                        <div className="flex flex-col">
                           ...
                           <span className="text-xs font-mono text-emerald-500">{new Date().toLocaleDateString()}</span>
                        </div>
                     </div>
                  </div>
                ) : null}
          ...
          <div className="space-y-8 sticky top-32 h-fit">
             <div className="premium-card bg-zinc-900 border border-white/5 p-8 rounded-xl space-y-8 shadow-2xl">
                ...
                <div className="space-y-4">
                   ...
                   {/* Step 2: Pay */}
                   <div className={`p-6 rounded-lg border transition-all ${status === 'paid' ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-white/5 bg-black/40'} ${status === 'draft' || status === 'sent' ? 'opacity-30 pointer-events-none' : ''}`}>
                      <div className="flex items-center justify-between mb-4">
                         <span className="text-[10px] font-black uppercase tracking-widest text-white flex items-center gap-2">
                            <CreditCard size={12} className={status === 'paid' ? 'text-emerald-500' : 'text-zinc-500'} />
                            02. Secure Payment
                         </span>
                         {status === 'paid' && <CheckCircle2 size={16} className="text-emerald-500" />}
                      </div>
                      ...
                      {status === 'signed' ? (
                        ...
                        <p className="text-[10px] text-emerald-500 font-bold uppercase tracking-widest">Payment Received</p>
                      ) : (
